import type { Property as PrismaProperty, Room as PrismaRoom } from "../generated/client.js"
...
export function toPropertyDTO(p: PrismaProperty & { rooms?: PrismaRoom[] }): PropertyDTO {
  const prices = (p.rooms ?? []).map(r => Number(r.pricePerMonth.toString())).filter(n => n > 0)
